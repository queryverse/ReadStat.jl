...
mutable struct ColumnBuf{T}
    values::Vector{T}
    isna::Vector{Bool}
    # Tags of tagged missing values ('a'-'z', '\0' where untagged), allocated
    # lazily the first time a tagged missing arrives in this column.
    tags::Union{Nothing,Vector{Char}}
end

newbuf(::Type{T}, n::Int) where {T} = ColumnBuf{T}(Vector{T}(undef, n), fill(true, n), nothing)
...
@inline function setvalue!(buf::ColumnBuf{T}, row::Int, v::T) where {T}
    ...
    elseif row == n + 1
        push!(buf.values, v)
        push!(buf.isna, false)
        buf.tags === nothing || push!(buf.tags, '\0')
    else
...
@inline function setmissing!(buf::ColumnBuf{T}, row::Int, tag::Char='\0') where {T}
    n = length(buf.values)
    if row <= n
...
function trim!(buf::ColumnBuf, n::Int)
    if length(buf.values) > n
        resize!(buf.values, n)
        resize!(buf.isna, n)
        buf.tags === nothing || resize!(buf.tags, n)
    end
    return buf
end
...
function getbuf(cols::TypedColumns, i::Int)
